...
const batchSize = Number(process.env.PROFIT_RECONCILE_BATCH_SIZE || 50);
...
if (!Number.isInteger(batchSize) || batchSize <= 0) {
  throw new Error('PROFIT_RECONCILE_BATCH_SIZE must be a positive integer');
}
...
const reconcileIntent = async (intent) => {
  ...
  if (!receipt) {
    console.log(`pending profit intent=${intent.id} tx=${intent.txHash}`);
    return;
  }

  if (Number(receipt.status) === 1) {
    await markConfirmed(intent.id);
    console.log(`confirmed profit intent=${intent.id} tx=${intent.txHash}`);
    return;
  }
...
const run = async () => {
  ...
  if (!Array.isArray(intents) || intents.length === 0) {
    console.log('no submitted profit intents to reconcile');
    return;
  }

  console.log(`reconciling ${intents.length} submitted profit intent(s)`);
