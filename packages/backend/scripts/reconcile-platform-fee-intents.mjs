...
const batchSize = Number(process.env.PLATFORM_FEE_RECONCILE_BATCH_SIZE || 50);
...
if (!Number.isInteger(batchSize) || batchSize <= 0) {
  throw new Error('PLATFORM_FEE_RECONCILE_BATCH_SIZE must be a positive integer');
}
...
const reconcileIntent = async (intent) => {
  ...
  if (!receipt) {
    console.log(`pending intent=${intent.id} tx=${intent.txHash}`);
    return;
  }

  if (Number(receipt.status) === 1) {
    await markConfirmed(intent.id);
    console.log(`confirmed intent=${intent.id} tx=${intent.txHash}`);
    return;
  }

  await markFailed(intent.id, 'Transaction reverted during reconciliation');
  console.error(`failed intent=${intent.id} tx=${intent.txHash} (receipt status ${receipt.status})`);
};
...
const run = async () => {
  ...
  if (!Array.isArray(intents) || intents.length === 0) {
    console.log('no submitted platform fee intents to reconcile');
    return;
  }

  console.log(`reconciling ${intents.length} submitted platform fee intent(s)`);
